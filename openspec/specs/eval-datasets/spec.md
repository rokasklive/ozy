...
Define the eval corpus — a synthetic-but-realistic downstream MCP catalog, labeled discovery gold sets, invocation and failure scenario sets, dataset schema and validation, and gold-set provenance/hygiene rules — that the eval harness loads and evaluates against.
