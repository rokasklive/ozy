...
Define Ozy's live brokered tool invocation: resolving a `toolRef` against
configuration, invoking the downstream tool via MCP `tools/call`, normalizing
results, and providing structured, repair-oriented failures — without requiring
a prior `ozy index` (SPEC.md §9.3).
