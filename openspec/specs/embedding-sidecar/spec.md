...
Define Ozy's optional Python embedding sidecar: the stdio JSONL protocol carried
over the sidecar's standard input/output, the FastEmbed CPU-only ONNX embedding
pipeline, embed-on-index ingestion of cataloged tools, semantic query-to-toolRef
ranking, and the sidecar's role as a non-authoritative derived store for the
embedding subsystem only (`SPEC.md` §10.4).
