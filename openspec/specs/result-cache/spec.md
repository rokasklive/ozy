...
Define Ozy's optional result cache implemented as a transparent decorator over the shared `Broker` interface. Both CLI and MCP adapter benefit without either path importing cache logic. The cache serves results for `findTool`, `describeTool`, and read-only `callTool` requests within a configurable TTL, with content-hash-based invalidation, and enforces a default-deny policy for write tools.
