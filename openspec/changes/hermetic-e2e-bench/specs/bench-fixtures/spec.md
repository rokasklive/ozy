...
### Requirement: Fixture metadata artifact

The generator SHALL write a machine-readable `fixture-meta.json` beside the
generated fixture containing at least the resolved culprit commit hash and the
generator version, and the runner SHALL consume it — the culprit hash reaches
grading through this artifact, never through hardcoded values.

#### Scenario: Metadata is written and consumed

- **WHEN** `ozy-bench fixture` completes and a live run is graded
- **THEN** `fixture-meta.json` exists with the culprit hash, and `grading.json` reflects a culprit check performed against that hash
