...
Single internal package that resolves every Ozy install location — binary,
config, data, cache, logs, Python venv, vector/model assets, and the user bin
directory — so the installer, uninstaller, sidecar provisioner, and runtime
config loading all agree on where things live, honoring platform conventions
(XDG on Linux/macOS, `%APPDATA%` / `%LOCALAPPDATA%` on Windows) and documented
environment overrides (e.g. `OZY_CONFIG`).
